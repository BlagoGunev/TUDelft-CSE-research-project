...
int robot, black, cycle_start, cycle_end, cycle_length;
...
vector<bool> col, vis, visited, b_o_w;





int ord(int x,int y){return (m * (x -1) + y);}
...

{
	...
	g.resize(MAXN), rg.resize(MAXN);

	col.assign(MAXN, 0), vis.assign(MAXN, 0), visited.assign(MAXN, 0);
...

{
...

	{
...

		{
			...
			for(int j = 1; j <= m; j ++)	col[ord(i, j)] = (s[j - 1] - '0') ^ 1;
...

		{
...

			{
				...
				if(s[j - 1] == 'U')	g[ord(i, j)].pb(ord(i - 1, j)), rg[ord(i - 1, j)].pb(ord(i, j)); 

				else if(s[j - 1] == 'D')	g[ord(i, j)].pb(ord(i + 1, j)), rg[ord(i + 1, j)].pb(ord(i, j));

				else if(s[j - 1] == 'R')	g[ord(i, j)].pb(ord(i, j + 1)), rg[ord(i, j + 1)].pb(ord(i, j));

				else if(s[j - 1] == 'L')	g[ord(i, j)].pb(ord(i, j - 1)), rg[ord(i, j - 1)].pb(ord(i, j));
		...
		//marking all the components

		function<void(int)> mark_comp = [&](int u)
...

		{
...

			{
				...
				b_o_w[dis[u] % cycle_length] = max((int)b_o_w[dis[u] % cycle_length], (int)col[u]);

				for(auto v : rg[u])

				{

					if(v != cycle_start and dis[v] == inf)
...

		{
...

			{
				...
				cycle_start = -1, cycle_end = -1, cycle_length = 1;



				dfs(u, dfs);

				for(int at = cycle_end; at != cycle_start; at = parent[at])
				...
				b_o_w.assign(cycle_length, 0);

				

				rdfs(cycle_start);

				for(int i = 0; i < cycle_length; i ++)	black += b_o_w[i];
