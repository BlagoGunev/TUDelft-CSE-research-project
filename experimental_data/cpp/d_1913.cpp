...
class MontgomeryModInt {
 public:
  ...
  friend constexpr bool operator<(const m32& lhs, const m32& rhs) { return lhs.get() < rhs.get(); }
...
int main() {
  ...
  while (testcase--) {
    ...
    std::vector<mint> dp(n + 1), sumdp(n + 1);
    mint sumstk = 0;
    std::vector<int> stk;
    stk.push_back(0);
    for (int i = 1; i <= n; ++i) {
      ...
      dp[i] = sumstk + sumdp[i - 1] - sumdp[stk.back()];
      if (stk.size() == 1) dp[i] += 1;
      sumdp[i] = sumdp[i - 1] + dp[i];
      stk.push_back(i);
      sumstk += dp[i];
    }
    mint ans = 0;
    for (int i : stk) ans += dp[i];
