...

{
  ...
  for(int i=0;i<n;i++) for(int j=0;j<n;j++) scani(in[i][j]);
  ...
  for(int i=0;i<v1.size();i++){
    ...
    if(mx < lf[x][y] + rt[x][y] - in[x][y]){

      mx = lf[x][y] + rt[x][y] - in[x][y];
  ...
  write(x1, ' '); write(y1, ' '); write(x2, ' '); write(y2);
