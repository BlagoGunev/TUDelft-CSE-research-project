...
int main() {
    ...
    while (t--) {
        ...
        for (ll i = 1; i <= m; i++) arr[i] += arr[i - 1];
        ll ans = 0, tot = m - arr[m];
        for (ll i = 1; i <= m; i++)
            if (arr[i] == arr[i - 1]) {
                ll tmp = tot + min(arr[i - 1], left) + min(arr[m] - arr[i], right);
                umx(ans, tmp);
            }
        umx(ans, tot + min(arr[m], right));
        umx(ans, tot + min(arr[m], left));
        cout << ans << nl;
    }
    return 0;
}