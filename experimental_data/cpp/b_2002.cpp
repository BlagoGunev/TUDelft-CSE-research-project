...
int main() {
	CASE(t) {
		int n; R(n);
		deque<int> a(n), b(n); R(a, b);

		bool bobWin = true;
		for (int i = 0; i < n - 1; i++) {
			set<int> boundaries = { b.front(), b.back() };
			if (!boundaries.count(a.front())) bobWin = false;
			if (!boundaries.count(a.back())) bobWin = false;

			if (!bobWin) break;

			if (b.front() == a.back()) b.pop_front(); else b.pop_back();
			a.pop_back();
		}
