...
typedef tree<int, null_type, less<int>, //could be int instead of node if you want
...
#define Tstart int t;cin>>t;while(t){solve();t--;}
...
class Btree{
  ...
  void update(ll k, ll val){   // imagine original array is 1-index and add the change (do not put the new value, put the 
...
segT_sum(vector<ll> A){
  ...
  for(int i=n-1; i>0; i--){
    ...
    T[i] = T[i<<1] + T[i<<1|1];   //build function
...
ll sum(ll l, ll r){    //return the sum [l, r)
...
segT_min(vector<ll> A){
  ...
  for(int i=n-1; i>0; i--){
    ...
    T[i] = min(T[i<<1] , T[i<<1|1]);   //build function
...
ll query(ll l, ll r){    //return the sum [l, r)
...
segT_max(vector<ll> A){
  ...
  for(int i=n-1; i>0; i--){
    ...
    T[i] = max(T[i<<1] , T[i<<1|1]);   //build function
...
void sleep(ll finalH, ll finalM, ll H, ll M){

  if(finalH > H){

    if(finalM >= M){

      cout << finalH - H << " " << finalM - M << endl;
      ...
      cout << finalH - H << " " << (60 - M) + finalM  << endl;

      return;

    }

  }else if(finalH == H && finalM >= M){

    cout << 0 << " " << finalM - M << endl;
