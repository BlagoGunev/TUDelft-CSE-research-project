...
public:
	...
	Segtree(const vector<T> &init): n(sz(init)), mn(4 << __lg(n)), mx(4 << __lg(n)), sum(4 << __lg(n)), tag(4 << __lg(n)) {
...
public:
	...
	auto Ask(int ql, int qr, T y) {
		...
		function<int(int, int, int)> dfs = [&](int i, int l, int r) {
			...
			return dfs(ls, l, mid) + dfs(rs, mid + 1, r);
