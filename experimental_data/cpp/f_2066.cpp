...
int main() {
    ...
    std::cin >> t;
    while (t--) {
        int aa[N], bb[M], ll[N], rr[N], ll_[N], rr_[N], ss[N], sss[M + 1][M + 1], lcp[N + 1][M + 1], dp[N + 1][M + 1][2], dq[N + 1][M + 1][2];
        char bad[N], used[N];
        ...
        for (h_ = 0; h_ < k; h_++) {
            ...
            if (dp[k][m][0] != -1 || dp[k][m][1] != -1) {
                ...
                for (h = 0; h < k; h++)
                    if (ss[h] >= s)
                        k_++;
                std::cout << k_ * 2 << std::endl;
                std::memset(used, 0, k * sizeof *used);
                while (1) {
                    h_ = -1;
                    for (h = 0; h < k; h++)
                ...
                while (1) {
                    ...
                    j = i + 1;
                    std::cout << i + 1 << " " << j << " " << rr_[h_] - ll_[h_] << std::endl;
