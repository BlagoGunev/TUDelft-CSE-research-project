// Problem: H. Thanos Snap
#pragma GCC optimize("Ofast,unroll-loops")
// Contest: Codeforces - Codeforces Global Round 25
// URL: https://codeforces.com/contest/1951/problem/H
// Memory Limit: 512 MB
// Time Limit: 3000 ms
// 
...
#endif
int a[1234567],b[2345678];
// void build(int o,int l,int r)
// {
	// if(l==r){b[o]=a[l];return;}
	// int m=l+r>>1;
// 	
// }
#define lc o*2
...
#define x first
#define y second
std::pair<int,int>chk(int o,int l,int r,int v,int t)
{
	t--;
	...
	int m=l+r>>1;
	auto L=chk(lc,l,m,v,t);
	auto R=chk(rc,m+1,r,v,t);
	std::pair<int,int>q={L.x+R.x,L.y+R.y};
	if(q.x&&q.y)q.x--,q.y--;
	return q;
	// if(L.x>0&&R.y>0)L.x--,R.y--;
	// else if(L.y>0&&R.x>0)L.y--,R.x--;
...
signed main()
{
	...
	for(int t=1;t<=n;t++)
	{
		...
		{	
			int m=l+r>>1;
			if(chk(1,1,1<<n,m,t+1).y==0)ns=m,l=m+1;
			else r=m-1;
		}
		odb(ns);
	}
