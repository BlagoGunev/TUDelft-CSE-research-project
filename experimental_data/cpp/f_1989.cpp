...
void Union(int x, int y){
    ...
    tot-=get_val(siz[x])+get_val(siz[y]);
    siz[x]+=siz[y],par[y]=x;
    tot+=get_val(siz[x]);
}
...
int n,m,q,scc[maxn];
ll res[maxn];
vector<int> V,adj[maxn],radj[maxn],ord;
bool vis[maxn];
...
void dfs(int u, int rt){
    vis[u]=1,de.pb(u),scc[u]=rt;
    for(auto v: adj[u]) if(!vis[v]) dfs(v,rt);
}

void solve(int l, int r, vector<array<int,3>> edges){
    int mid=l+r>>1;
    V.clear();
    ...
    for(auto u: V) vis[u]=0,adj[u].clear(),radj[u].clear();
    for(auto& [u,v,i]: edges) if(i<=mid) adj[u].pb(v),radj[v].pb(u);
    ord.clear();
    for(auto u: V) if(!vis[u]) rdfs(u);
    ...
    for(auto u: ord) if(!vis[u]){
        de.clear();
        dfs(u,u);
        if(l==r) for(int i=0; i+1<de.size(); ++i) tmp.pb({de[i],de[i+1]});
    }
    if(l==r){
        for(auto [u,v]: tmp) Union(u,v);
        res[l]=tot;
        return;
    }
    for(auto& [u,v,i]: edges){
...
void ahcorz(){
    cin >> n >> m >> q;
    for(int i=0; i<n+m+q; ++i) par[i]=i,siz[i]=(i>=n+m?0:1);
    vector<array<int,3>> edges(q*2);
    for(int i=0; i<q; ++i){
        //int u,v; cin >> u >> v; u--,v--;
        int x,y; char c; cin >> x >> y >> c; x--,y--;
        if(c=='R'){
            edges[i*2]={n+m+i,n+y,i*2};
            edges[i*2+1]={x,n+m+i,i*2+1};
        }
        else{
            edges[i*2]={n+y,n+m+i,i*2};
            edges[i*2+1]={n+m+i,x,i*2+1};
        }
        //edges[i]={u,v,i};
    }
    solve(0,q*2-1,edges);
    for(int i=0; i<q; ++i) cout << res[i*2+1] << "\n";
}
