...
void solve() {
    ...
    if (R == 1 || C == 1) {
        FOR(r, 0, R) FOR(c, 0, C) dat[r][c] = 'X';
        return pvn(dat);
    }

    // FOR(r, 0, R) { dat[r][0] = 'X'; }

    // handle the special case with c=1.
    ...
    //         dbgc("special", r);
    //         dat[r][0] = '.';
    //         if (r > 0) {dat[r-1][1] = 'X'; dat[r-1][0] = '.';}
    //         if (r < R-1) {dat[r+1][1] = 'X'; dat[r+1][0] = '.';}
    //         ++r;
    //     }
    // }
    // dbg(pdh(dat));
    ...
    int row_killed = 0;
    if (R % 3 == 0) {row_killed = 1;}
    while (row_killed < R) {
        FOR(c, 0, C) { dat[row_killed][c] = 'X'; }
        if (row_killed > 1) {
            if (dat[row_killed-1][1] == 'X' || dat[row_killed-2][1] == 'X') {
                dat[row_killed-1][1] = 'X'; dat[row_killed-2][1] = 'X';
            } else {
                dat[row_killed-1][0] = 'X'; dat[row_killed-2][0] = 'X';
            }
        }
        row_killed += 3;
    }
    ...
    auto check = [&]() {
        ...
        FOR(r, 0, R-1) FOR(c, 0, C-1) {
            for(auto [dr, dc] : V<pll>({{1, 0}, {0, 1}})) {
                int rr = r + dr;
                int cc = c + dc;
                if (dat[r][c] == 'X' && dat[rr][cc] == 'X') {
                    if (!dsu.unite(r*C + c, rr*C + cc)) {
                        assert(false);
                    }
                }
            }
