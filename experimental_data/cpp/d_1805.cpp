...
int firstMax[200010];   // to store first-max length.
int secondMax[200010];  // to store second-max length.
int c[200010];          // to store child for path of max length.
...
int main() {
    ...
    dfs(1, -1);
    dfs2(1, -1);
    sort(firstMax+1,firstMax+n+1);
    for (int k=1;k<=n;k++)
    {
        auto it=lower_bound(firstMax,firstMax+n,k);
        cout<<it-firstMax<<' ';
    }
}