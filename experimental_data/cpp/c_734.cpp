...
int main()
{
    scani3(n, m, k);
    scani2(x, s);
    for(int i=0;i<m;i++) scani(a[i]);
    for(int i=0;i<m;i++) scani(b[i]);
    for(int i=0;i<k;i++) scani(c[i]);
    for(int i=0;i<k;i++) scani(d[i]);
    LL tp = lowerBound(s);
//    cout << "no " << tp << endl;
    LL mn = (n - tp) * x;
//    cout << mn << endl;
