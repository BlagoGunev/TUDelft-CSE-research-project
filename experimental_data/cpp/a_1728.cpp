...
void sweetheart( /*I'M THE BEST*/ ) {
    ...
    cout<<max_element(v.begin(),v.end())-v.begin()+1<<endl; //" "<<min_element(v.begin(),v.end())-v.begin()+1<<"\n";
...
vi fact(1000001),factulta(1000001),ulta(1000001);

int pnc(long long x,long long y) {
    ...
    return(fact[x]*factulta[y]%mod10*factulta[x-y]%mod10);
...
void calc() {
    ...
    fact[i]=fact[i-1]*i%mod10;

    rep(i,2,1000001)

    ulta[i]=mod10-ulta[mod10%i]*(mod10/i)%mod10;

    rep(i,2,1000001)

    factulta[i]=factulta[i-1]*ulta[i]%mod10;
