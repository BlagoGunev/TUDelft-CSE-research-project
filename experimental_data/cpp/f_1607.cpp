...
namespace Template {
    ...
    template<typename T> istream &operator >> (istream &in, vc<T> &v) {for (auto &t : v) in >> t;return in;}

    template<typename T> istream &operator >> (istream &in, vvc<T> &v) {for (auto &vv : v) for (auto &t : vv) in >> t;return in;}
...

{
    ...
    for(int i = 1; i <= n; ++i)
...

        {
            ...
            if(c == 'L' && j != 1) to[ed] = ed - 1;

            else if(c == 'R' && j != m) to[ed] = ed + 1;

            else if(c == 'U' && i != 1) to[ed] = ed - m;

            else if(c == 'D' && i != n) to[ed] = ed + m;
