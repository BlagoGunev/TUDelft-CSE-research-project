...
//int some_primes[7] = {24443, 100271, 1000003, 1000333, 5000321, 98765431, #define T18         1000000000000000000LL
...
vector<int> getAns(int x, int y) {
    ...
    q.push_back({y, -1});
    hist.resize(2);
    for (int qc = 0; qc < (int)q.size(); ++qc) {
        auto pr = q[qc];
//        DBN(pr);
        int cur = pr.first;
        int t = cur / BS;
        auto s = sign(pr.second);
        auto where = s == 1 ? 0 : 1;
        if (used[1 - where][t]) {
            for (int i = 0; i < qc; ++i) {
                if (q[i].first / BS == t && sign(q[i].second) != s) {
                    int what = q[i].first;
                    if (s < 0) swap(qc, i), swap(what, cur);
                    int pos = i;
                    vector<int> ans2, ans;
                    while (pos > 1) {
                        ans2.push_back(hist[pos].first);
                        pos = hist[pos].second;
                    }
                    pos = qc;
                    while (pos > 1) {
                        ans.push_back(hist[pos].first);
                        pos = hist[pos].second;
                    }
                    reverse(all(ans));
//                    DBN(cur, what, cur - what);
                    while (cur < what) {
                        ans.push_back(1);
                        ++cur;
                    }
...
//            return -1;
//            return abs(pr.second);
        }
        used[where][t] = true;
        auto res = pr.second + sign(pr.second);
        int old = hist[qc].first;
        if (old != 2) {
            q.push_back({(cur + 1) % p, res}); hist.push_back({1, qc});
        }
        if (old != 1) {
            q.push_back({(cur + p - 1) % p, res}); hist.push_back({2, qc});
        }
        if (old != 3) {
            q.push_back({powmod(cur, p - 2, p), res}); hist.push_back({3, qc});
        }
//        assert(hist.size() == q.size());
    }
    return {};
}
int solve() {
