// time-limit: 1000
// problem-url: https://codeforces.com/contest/1686/problem/C
// clang-format off
#include"bits/stdc++.h"
...
int main()
{
    ...
    while (t--) {
        ...
        bool ch = true;
        for (int i = 0; i < n and ch; i++) {
            bool p = false;
            if ((ans[i] > ans[(i + 1) % n] and ans[i] > ans[(i + n - 1) % n]) or (ans[i] < ans[(i + 1) % n] and (ans[i] < ans[(i + n - 1) % n]))) {
                p = true;
            }
            ch &= p;
        }
