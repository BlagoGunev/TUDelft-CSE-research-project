...
int main()
{
    auto solve = [&]() {
        ...
        while (bs.size() > 1) {
            void(0);
            Vec<int> nbs;
            if (Z > 0) { nbs.push_back(bs[0]); }
            int nZ = std::max(0, Z - 1);
            for (int i : rep((int)bs.size() - 1)) {
                if (bs[i + 1] == bs[i]) {
                    nZ++;
                } else {
                    nbs.push_back(bs[i + 1] - bs[i]);
                }
            }
            sortAll(nbs);
            std::swap(bs, nbs);
        ...
        return out.ln(bs.empty() ? 0_i64 : bs[0]);
    };
    const auto T = in.val<int>();
    for (auto _temp_name_3 [[maybe_unused]] : rep(T)) {
        solve();
    }
    return 0;
}