...
void solve(){
    ...
    mint::set_mod(P);
    mod=P;
    vvvm DP(N+2,vvm(K+1,vm(2,0)));
    DP[0][0][0]=1;
    rep(n,N+1){
        vm SDP(2,0);
        rep(k,K+1)rep(j,2)SDP[j]+=DP[n][k][j];
        rep(k,K+1){
            rep(j,2)DP[n+1][k][j]+=SDP[j];
        }
        if(n==N)break;
        vvm AD(K+1,vm(2,0));
        rep(c,K){
            rep(j,2)AD[K-c-1][j]+=DP[n][c][j];
        }
        rep(d,2)for(ll c=K-1;c>=0;c--)rep(j,2)AD[c][j]+=AD[c+1][j];
        rep(c,K+1)rep(j,2){
            DP[n+2][c][1-j]+=AD[c][j];
        }
    }
    mint an=DP[N+1][0][0]-DP[N+1][0][1];
    cout<<an.val()<<endl;
}
