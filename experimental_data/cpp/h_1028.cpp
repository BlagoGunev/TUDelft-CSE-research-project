...
std::vector<pii>q[N];
int n,m,pm,s[N],ans[M],p[V/10],lo[V],lp[V],f[7],fc,fs[133],g[V][8],ua[15];
char mu[V];
...
int main()
{
	fo(i,2,V-1)
	{
		...
		{
			lp[i*p[j]]=p[j];
			if(lo[i]%p[j]==0)lo[i*p[j]]=lo[i]/p[j];
			else lo[i*p[j]]=lo[i]*p[j];
			if(i%p[j]==0)
			{
				break;
			}
	...
	fo1(i,n)
	{
		...
		fo0(j,1<<fc)
		{
			...
			//out,i,' ',x,' ',y,'\n';
			fo0(k,8)repr(ua[k+y],g[x][k]);
			g[x][y]=i;
		}
		foe(j,q[i])
		{
			fo0(k,15)if(ua[k]>=j->xx)
			{
				ans[j->yy]=k;
				break;
			}
