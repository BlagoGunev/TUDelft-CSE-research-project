...
inline void solve(){
	...
	rd(n);
	int i,j,x;
	for(i=0;i<n;i++) rd(x),cnt[x]++;
	bool have=0;
	ll wz;
	long double mn=1000000000.0,t;
...
//				putchar(':');
	...
	for(i=1;i<v.size();i++){
		t=(double)(4*(pow(v[i]-v[i-1],2)))/(double)(v[i-1]*v[i-1]+(v[i]-v[i-1])*v[i-1]);
		if(t<mn){
			mn=t;
			wz=i;			
		}
	}
	_write(v[wz-1]),_write(v[wz-1]),_write(v[wz]),_write(v[wz]);
	putchar('\n');
}
int main(){
