...
void solve(){
    ...
    for(auto &[w, i] : W){
        ...
        ll cnt = dp[i].second;
        fori(j, n){
            if(j != n - 1 && WW[j] <= WW[i]) continue;
            if(D[i][j] == inf) continue;
            ll cc = max(0LL, (D[i][j] - cost + w - 1) / w);
            ll ncost = cost + cc * w - D[i][j];
            ll ncnt = cnt + cc;
            if(ncnt < dp[j].second) dp[j] = {ncost, ncnt};
            else if(ncnt == dp[j].second) chmax(dp[j].first, ncost);
        }
    }
    print(dp[n - 1].second);
}
