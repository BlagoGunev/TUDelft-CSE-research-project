...
ll res=0;
 ...
 FOR(i,0,n)
 {
  r(p);
  s=p[0];
  FOR(j,1,sz(p)) if (p[j]==')' && s.back()=='(') s.resize(sz(s)-1); else s.pb(p[j]);
  a=b=0;
  FOR(i,0,sz(s)) { if (s[i]=='(') a++; else b++; if (a>0 && b>0) break; }
  D(s,a,b);
  if (a==0 && b==0) c++;
  else if (a==0) x[b]++;
  else if (b==0) y[a]++;
 ...
 D(c);
 res=1ll*c*c;
 if (sz(x)<sz(y)) tr(x,it) res+=1ll*it->second*y[it->first];
 else tr(y,it) res+=1ll*it->second*x[it->first];
 wl(res);
