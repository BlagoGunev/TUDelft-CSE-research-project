...
void solve() {
    ...
    vvi v(n);
    rep(i, n) {
        rrep(j, 30) if(s[i] >> j & 1) v[i].pb(j);
        while(SZ(v[i]) < 30) v[i].pb(-1);
    }
    vi io(n);
    iota(all(io), 0);
    mint ans;
    rep(i, n) {
        int L, R;
        {
            int l = 0;
            rep(j, 30) {
                int nl = lower_bound(io.begin() + l, io.begin() + i, v[i][j], [&](int val, int key){ return v[val][j] < key; }) - io.begin();
                ans += mint(nl - l) * 2 * (j / 2 + 1);
                l = nl;
            }
            L = l;
        }
        {
            int r = n;
            rep(j, 30) {
                int nr = upper_bound(io.begin() + i + 1, io.begin() + r, v[i][j], [&](int key, int val){ return key < v[val][j]; }) - io.begin();
                ans += mint(r - nr) * (2 * ((j + 1) / 2) + 1);
                r = nr;
            }
            R = r;
        }
        ans += R - L;
        rep(j, 30) if(v[i][j] != -1) ans += R - L;
    }
    ans /= (ll)n * n;
    print(ans);
}
