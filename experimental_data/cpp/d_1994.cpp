...
namespace outputFormat{
	...
	inline string YNcap(bool b) { return b ? "YES" : "NO"; }
...
int main() {
	CASE(t) {
		...
		for (int phase = n - 1; phase >= 1; phase--) {
			vector<vector<int>> buckets(phase);
			for (int i = 0; i < n; i++)
				if (!died[i])
					buckets[a[i] % phase].emplace_back(i);

			int take = -1;
			for (int i = 0; i < phase; i++)
				if ((int)buckets[i].size() >= 2)
					take = i;

			assert(take != -1);
			
			ans[phase - 1] = make_pair(buckets[take][0] + 1, buckets[take][1] + 1);
			died[buckets[take][0]] = true;
		}
