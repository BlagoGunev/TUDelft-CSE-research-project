...
int suf1[2][maxn][maxn],suf2[2][maxn][maxn];
int dp1[2][maxn][maxn],dp2[2][maxn][maxn];
...
inline void solve1()
{
    ...
    for (int i=0;i<=1;i++)
    {
        for (int j=0,k,len;j<n;j++)
        {
            k=j;len=0;
            while (k>=0)
            {
                if (g[i][k]!=w[len++]||(len<<1)>m)
                    break;
                if (k!=j)
                    left1[i^1][j+1][len<<1]=k+suf1[i^1][k][len]>j;
                k--;
            }
        }
        for (int j=n-1,k,len;j>=0;j--)
        {
            k=j;len=m;
            while (k<n)
            {
                if (g[i][k]!=w[--len]||(len<<1)<m)
                    break;
                if (j!=k)
                    right1[i^1][j][(len<<1)-m]=j+suf1[i^1][j][(len<<1)-m]>k;
                k++;
            }
        }
    }
            ...
            for (int k=0;k<=m;k++)
            {
                ...
                if (dp2[i][j][k])
                {
                    if (j<n)
                    {
                        ...
                        if (k+1<m&&g[i][j]==w[k]&&g[i^1][j]==w[k+1])
                        {
