...
bool solve(vll S){
    ...
    for(int i=1;i<=N;i++){
        po[i-1]=q;
        rui[i]=rui[i-1]+q*(S[i-1].se-S[i-1].fi+1);
        rui[i]%=mod;
        q*=p;
        q%=mod;
    }
    ...
    for(ll dh=1;dh<N;dh++){
        ...
        for(int i=0;i<N;i+=dh){
            int l=i,r=min(N,i+dh);
            if((i/dh)%2==0) sum+=(rui[r]-rui[l])*po[N-1-i]%mod;
            else sum-=(rui[r]-rui[l])*po[N-1-i]%mod;
        }
        sum%=mod;
        //cout<<sum<<endl;
        if(sum) continue;
        ...
        /*
         cout<<dh<<endl;
         for(auto [a,b]:A) cout<<a<<" "<<b<<endl;
         cout<<endl;
         for(auto [a,b]:B) cout<<a<<" "<<b<<endl;
         cout<<endl;
         */
