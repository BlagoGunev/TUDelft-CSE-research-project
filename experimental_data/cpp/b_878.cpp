...
int main() {
	ll n, k, m; reader(n, k, m);
	vector<ll> a(n);
	FOR(i, n) reader(a[i]);

	vector<Pll> q;
	FOR(i, n) {
		if (sz(q) && q.back().first == a[i]) {
			q.back().second++;
		} else {
			q.emplace_back(a[i], 1);
		}
		if (sz(q) && q.back().second == k) {
			q.pop_back();
		}
	}
	...
	if (m == 1) {
		...
		if (sz(mdl) == 1 && !sk) {
			vector<Pll> cur;
			FOR(i, r + 1) cur.push_back(q[i]);
			ll rem = ll(mdl[0].second) * (m - 2) % k;
			cur.emplace_back(mdl[0].first, rem);
			for (int i = l; i < sz(q); i++) cur.emplace_back(q[i]);

			vector<Pll> q2;
			for (auto& kv : cur) {
				if (sz(q2) && q2.back().first == kv.first) {
					q2.back().second += kv.second;
				} else {
					q2.emplace_back(kv);
				}
				if (sz(q2) && q2.back().second >= k) {
					q2.back().second -= k;
					if (q2.back().second == 0) {
						q2.pop_back();
					}
			...
			ans += rcnt * (m - 2);
			for (int i = l; i < sz(q); i++) ans += q[i].second;
		} else {
			for (auto& kv : q) ans += kv.second;
			ans *= m;
			ans -= ll(remcnt) * k * (m - 1);
