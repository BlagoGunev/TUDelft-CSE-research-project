...
vector<vector<ll>> calc_binomail(int n, int m) {
    vector<vector<ll>> res(n + 1, vector<ll>(m + 1));
    res[0][0] = 1;
    for(int i = 1; i <= n; i++) {
        res[i][0] = 1;
        for(int j = 1; j <= min(i, m); j++) res[i][j] = res[i - 1][j - 1] + res[i - 1][j];
    }
    return res;
}
...
int main() {
    ...
    rep(i, n + 1) zeta_supersetsum(f[i]);
    rep(i, n + 1) dump(f[i]);
    vi dp(1 << n);
    rep(i, n + 1) rep(j, 1 << n) if(popcount(j) == i) dp[j] += f[i][j];
    zeta_subsetsum(dp);

    vi ans;
    rep(i, 1 << n) {
        if(dp[i] == (1 << n) - 1) ans.eb(i);
    }
    OUT(si(ans));
    fore(e, ans) OUT(e);
}