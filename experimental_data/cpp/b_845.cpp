...
int k  , x;
vector <pair<int, bool> > del;


int calcans(int leftsum , int rightsum)
{
    int ans = 0;
    while (leftsum < rightsum)
    {
        if (del.back().second)
        {
            rightsum-= del.back().first;
        }
        else
        {
            leftsum+= del.back().first;
        }
        del.pop_back();
        ans++;
    }
...
int main()
{
    ...
    if (left >= right + 1)
    {
        for (int i = 0; i < 6; i++)
        {
            int a = del[i].first;
            del[i].first = 9 - a;
            if(del[i].second) del[i].second = 0;
            else del[i].second = 1;
        }
            swap(left ,right);
    }
