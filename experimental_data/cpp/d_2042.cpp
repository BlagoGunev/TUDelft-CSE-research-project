...
void solve()
{
    ...
    for (int i = 0; i<n; i++)
    {
        int l, r; cin>>l>>r; a[i] = {l, r};
    }

    vector<int> ans_r(n, INF), ans_l(n, -1);


    vector<pair<pair<int, int>, int>> a_right;
    for (int i = 0; i<n; i++) a_right.push_back({mp(a[i].first, -a[i].second), i});
    sort(a_right.begin(), a_right.end());
    for (auto &it: a_right) it.first.second*=-1;

    set<int> right_set;
    for (auto pr: a_right)
    {
        auto it = right_set.lower_bound(pr.first.second);
        if (it != right_set.end()) ans_r[pr.second] = *it;
        right_set.insert(pr.first.second);
    }

    vector<pair<pair<int, int>, int>> a_left;
    for (int i = 0; i<n; i++)
    {
        a_left.push_back({mp(a[i].second, -a[i].first), i});
    }
    sort(a_left.begin(), a_left.end());
    for (auto &it: a_left) it.first.second*=-1;
    reverse(a_left.begin(), a_left.end());

    set<int> left_set;
    for (auto pr: a_left)
    {
        auto it = left_set.lower_bound(pr.first.second+1);
        if (it != left_set.begin())
        {
            it--; ans_l[pr.second] = *it;
        }
        left_set.insert(pr.first.second);
    }

    /*print(ans_r); print(ans_l);
    cout<<endl;*/
    ...
    for (int i = 0; i<n; i++)
    {
        if (ans_r[i] == INF) cout<<0<<endl;
        else cout<<(ans_r[i] - ans_l[i]) - (a[i].second - a[i].first)<<endl;
    }
    //cout<<endl;
}
