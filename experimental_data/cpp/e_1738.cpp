...
mint solve() {
  ...
  F0R(i, N) {
    ...
    pfx[i] = A[i] + (i == 0 ? 0 : pfx[i-1]);

    if (pfx[i] * 2 < total) mp1[pfx[i]]++;

    else if (pfx[i] * 2 == total) middle++;

  }

  F0Rd(i, N) {

    sfx[i] = A[i] + (i == N-1 ? 0 : sfx[i+1]);

    if (sfx[i] * 2 < total) mp2[sfx[i]]++;
  ...
  for (auto [x, c]: mp1) {
    ...
    if (mp2.count(x)) blocks.push_back({c, mp2[x]});
