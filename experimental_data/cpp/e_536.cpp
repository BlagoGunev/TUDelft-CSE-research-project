...
void dfs(int nd,int pr){
	...
	P[nd][0]=pr;lvl[nd]=(pr==-1?1:lvl[pr]+1);sub[nd]=1;
...
ll atla(int u,int v){
	...
	while(u!=r and u!=-1){
		...
		if(rch==chain[u]){
			...
			sag=merge(tap(pos[r]+1,pos[u],1,1,TIM),sag);

			break;

		}

		sag=merge(tap(pos[ata[chain[u]]],pos[u],1,1,TIM),sag);
	...
	while(v!=r and v!=-1){
		...
		if(rch==chain[v]){
			...
			cep=merge(tap(pos[r]+1,pos[v],1,1,TIM),cep);

			break;

		}

		cep=merge(tap(pos[ata[chain[v]]],pos[v],1,1,TIM),cep);
