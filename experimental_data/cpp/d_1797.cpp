...
vector<int>a(1e5+10),st(1e5+10,0),imp(1e5+10,0),parent(1e5+10);
vector<set<pii>>vpq(1e5+10);
...
void solve(int ttc){
    ...
    while(q--){
        ...
        if(t==1){
            ...
            pii tp=*vpq[x].rbegin();
            int hs=-1*tp.S;
            vpq[parent[x]].erase({st[x],-1*x});
            vpq[parent[x]].insert({st[x],tp.S});
            st[x]-=tp.F;
            imp[x]-=imp[hs];
            vpq[x].erase(tp);
            vpq[hs].insert({st[x],-1*x});
            st[hs]+=st[x];
            imp[hs]+=imp[x];
            parent[hs]=parent[x];
            parent[x]=hs;
