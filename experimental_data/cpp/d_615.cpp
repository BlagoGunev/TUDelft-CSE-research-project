...
{
	get1(n);
	for(int i=1,p;i<=n;i++){get1(p);cnt[p]++;}
	LL k=1,ans=1;
	map<int,int>::iterator it;
	for(it=cnt.begin();it!=cnt.end();it++)
	{
		if(!use&&it->second%2==1)
		{
			k=(k*(it->second+1)/2)%(mod-1);
			use=true;
		}
		else k=(k*(it->second+1))%(mod-1);
	}
	for(it=cnt.begin();it!=cnt.end();it++)ans=ans*power(it->first,k*it->second/(use?1:2))%mod;
	printf("%I64d\n",ans);
	return 0;
}