...
vector<pair<int,int>> arr[500005];
vector<int> cnt(200001);
int vis[500001],dis[200001],in[200001],out[2000001];
void bfs(int v){
    queue<int> q;
    q.push(v);
    vis[v]=1;
...
vector<int> temp;
vector<int> parent(2*1e5+10,-1);
vector<int> colour(200000);
vector<int> dp(2e5+10,1);
int cff=0;
void dfs(int node,vector<vector<int>>& v,int ip){
    vis[node]=1;
    for(int child:ar[node]){
        if(vis[child]==0){
            dfs(child,v,ip);
            for(int j=0;j<32;j++){
                v[node][j]=v[node][j]+v[child][j];
            }
        }
    }
    int ff=0;
...
// // // //     return true;
// // // // }
unsigned long long power(unsigned long long x,int y,int m5)
{
    unsigned long long res = 1; // Initialize result
    x = x%m5; // Update x if it is more than or
    // equal to p
...
// }
// //O(logn)
// void point_update(int start,int end,int index,vector<int>& sgt,int pos,int val){
...
//     int mid=(start+end)/2;
//     if(pos<=mid){
//         point_update(start,mid,left(index),sgt,pos,val);//update left side
//     }
//     else{
//         point_update(mid+1,end,right(index),sgt,pos,val);//update right side;
//     }
...
signed main(){
    ...
    for(int i=0;i<t;i++){
        ...
        for(int i=2;i<n-2;i++){
            if(s.at(i-2)=='0' && s.at(i)=='0' && s.at(i+2)=='0' && t.at(i-1)=='0' && t.at(i+1)=='0'){
                v1.pb(i);
            }
        }
        for(int i=1;i<n-2;i++){
            if(s.at(i)=='0' && t.at(i-1)=='1' && s.at(i+2)=='0' && t.at(i+1)=='0'){
                v2.pb(i);
            }
        }
        for(int i=2;i<n-1;i++){
        ...
        for(int i=1;i<n-1;i++){
            if(s.at(i)=='0' && t.at(i+1)=='1' && t.at(i-1)=='1'){
                v4.pb(i);
            }
        }
        // for(auto it:ori){
        ...
        for(int i=0;i<q;i++){
            ...
            cin>>l>>r;
            int ans=0;
            auto it=lower_bound(ori.begin(),ori.end(),l-1);
            auto it1=lower_bound(ori.begin(),ori.end(),r);
            ans+=(it1-it);
            it=lower_bound(v1.begin(),v1.end(),l+1);
            it1=lower_bound(v1.begin(),v1.end(),r-2);
            int ff=(it1-it);
            ans+=max(0ll,ff);
            it=lower_bound(v2.begin(),v2.end(),l);
            it1=lower_bound(v2.begin(),v2.end(),r-2);
            ff=(it1-it);
            ans+=max(0ll,ff);
            it=lower_bound(v3.begin(),v3.end(),l+1);
            it1=lower_bound(v3.begin(),v3.end(),r-1);
            ff=(it1-it);
            ans+=max(0ll,ff);
            it=lower_bound(v4.begin(),v4.end(),l);
            it1=lower_bound(v4.begin(),v4.end(),r-1);
            ff=(it1-it);
            ans+=max(0ll,ff);
            cout<<ans<<'\n';
        }
