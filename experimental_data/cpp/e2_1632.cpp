...
void solve() noexcept {
  ...
  algo::heavy_light_decomposition<> hld(adj);
  vector<int> dist(n);
  for (int u = 0; u < n; ++u) { dist[u] = hld.depth(u); }
  auto it = max_element(dist.begin(), dist.end());
  const int s = it - dist.begin();
  const int dmax = *it;
  vector<vector<int>> ds(dmax + 1);
  for (int i = 0; i < n; ++i) { ds[dist[i]].emplace_back(i); }
  ...
  debug("dx = {}\n", dx);
  for (int d = 0; d < dmax; ++d) { dx[d] = d - (dx[d] + 1) / 2; }
  debug("dx = {}\n", dx);
