...
void solve() {
	...
	F0R(i, N) {
		...
		ll opt2 = min(R1 + R2, R1*A[i] + 2*R1)+D;

		dp[i+1][0] = min(opt2 + dp[i][1], opt1 + min(dp[i][0], dp[i][1]+cost));

		dp[i+1][1] = opt2+dp[i][0];

	}

	cout << min(dp[N][0], dp[N][1]+D)+(N-1)*1LL*D;
