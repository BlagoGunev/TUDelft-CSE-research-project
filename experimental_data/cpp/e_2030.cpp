...
int main(){
    ...
    int _=1;cin>>_;
    vm ex(2e6,1),re(2e6),two(2e6,1);
    FOR(i,1,2e6)ex[i]=i*ex[i-1];
    REP(i,2e6)re[i]=1/ex[i];
    FOR(i,1,2e6)two[i]=two[i-1]*2;
    while(_--){
      ll N;cin>>N;
      vi T(N);
      REP(i,N){
      ...
      vi B(N+1);
      REP(i,N)B[i+1]=T[i]+B[i];
      mint ans=0;
      vm DP(T[0]+1);
      REP(i,T[0]+1)DP[i]=ex[T[0]]*re[i]*re[T[0]-i];
      RREP(i,T[0])DP[i]+=DP[i+1];
      FOR(i,1,T[0]+1)ans+=DP[i]*two[B[N]-B[1]];
      FOR(i,1,N){
        ll t=min(T[i]+1,sz(DP));
        vm binom(T[i]+1);
        REP(j,T[i]+1)binom[j]=ex[T[i]]*re[j]*re[T[i]-j];
        vm cal=binom;
        RREP(j,T[i])cal[j]+=cal[j+1];
        vm _DP(t);
        REP(j,t)_DP[j]=DP[j]*cal[j];
        FOR(j,1,t)ans+=_DP[j]*two[B[N]-B[i+1]];
        DP=_DP;
      }
      cout<<ans.val()<<endl;
    }
