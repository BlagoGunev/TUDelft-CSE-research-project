...
vi minplus_convolution(vi&b,vi&a){
  REP(i,sz(b)-2)assert(b[i+1]-b[i]<=b[i+2]-b[i+1]);
  vi c(sz(a)+sz(b)-1,1e18);
  function<void(ll,ll,ll,ll)>f=[&](ll l,ll r,ll x,ll y){
    if(l>=r)return;
    ll m=(l+r)/2,argmin;
    FOR(i,x,y)if(0<=m-i&&m-i<sz(b)&&chmin(c[m],a[i]+b[m-i]))argmin=i;
    f(l,m,x,argmin+1);
    f(m+1,r,argmin,y);
  };
  f(0,sz(c),0,sz(a));
...
int main(){
    ...
    while(_--){
        ...
        while(Q--){
          ll l,r;cin>>l>>r;l--;
          if(B[r][0]-B[l][0]){
            cout<<r-l-1<<" "<<B[r][0]-B[l][0]<<endl;
            continue;
          }
          if(r-l>50){
            mint ans=0;
            REP(i,51)ans+=(B[r][i]-B[l][i])*(B[r][i]-B[l][i]-1)/2;
            cout<<r-l-2<<" "<<ans.val()<<endl;
            continue;
          }
          vvi T(r-l+1,vi(64,1e18));
          FOR(i,l,r){
            T[i-l+1]=T[i-l];
            chmin(T[i-l+1][A[i]],1LL);
            REP(j,64)chmin(T[i-l+1][j^A[i]],T[i-l][j]+1);
          }
          if(T[r-l][0]>1e17)cout<<-1<<endl;
          else{
            vector<mint>DP(64,0);
            FOR(i,l,r){
              vector<mint>EP(64,0);
              REP(j,64)if(T[i-l][j]==T[i-l+1][j])EP[j]+=DP[j];
              if(T[i-l+1][A[i]]==1)EP[A[i]]+=1;
              REP(j,64)if(T[i-l+1][j^A[i]]==T[i-l][j]+1)EP[j^A[i]]+=DP[j];
              DP=EP;
            }
            cout<<r-l-T[r-l][0]<<" "<<DP[0].val()<<endl;
          }
        }
    }
    return 0;
