...
void solve(){
    ...
    for (int i = 0 ; i < m ; ++i){
        RD(b[i].u , b[i].v , b[i].w);
    }
    sort(b , b + m);
    for (int i = 0 ; i < m ; ++i){
        int u = b[i].u , v = b[i].v , w = b[i].w;
        if (dp[u].now.fi == w) dp[v].Update(MP(w , dp[u].pre.se + 1));
        else{
            dp[v].Update(MP(w , dp[u].now.se + 1));
            dp[v].Update(MP(w , dp[u].pre.se + 1));
        }
//        swap(u , v);
//        if (dp[u].now.fi == w) dp[v].Update(MP(w , dp[u].pre.se + 1));
//        else{
...
//        }
    ...
    int ans = 0;
    for (int i = 1 ; i <= n ; ++i)
        checkMax(ans , max(dp[i].now.se , dp[i].pre.se));
    OT(ans);
}
int main(){
