...
int main(void)
{
	...
	while(T--){
		...
		mint ans = 0, ans2 = 0;
		{
			mint coe = mint(ceil(n-k+1, 2))/mint(n-k+1), coe2 = mint((n-k+1)/2)/mint(n-k+1);
			rep(i, 1, k) ans += coe * a[i], ans2 += coe2 * a[i];
		}
		{
			mint coe = mint(ceil(n-k, 2))/mint(n-k), coe2 = mint((n-k)/2)/mint(n-k);
			rep(i, k+1, n) ans += coe * a[i], ans2 += coe2 * a[i];
		}
		outl(ans, ans2);
	}
