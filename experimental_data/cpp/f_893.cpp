...
vector<int> v[101010];
int curT = 0;
vector<int> vh[101010];
void go(int cur, int p = -1, int g = 0) {
    vh[g].push_back(cur);
    h[cur] = g;
    tin[cur] = ++curT;
    for (int to : v[cur]) {
...
//    DBN(cur, l, r, y);
    ...
    if (l == r) {
        ...
        int m = (l + r) / 2;
        if (x <= m) {
            update(cur->lp, l, m, x, y);
        } else {
            update(cur->rp, m + 1, r, x, y);
        }
        cur->val = min(cur->lp ? cur->lp->val : INF, cur->rp? cur->rp->val : INF);
    }
}
int getMin(Node* cur, int l, int r, int x, int y) {
//    DBN(cur, l, r, x, y);
    if (!cur) return INF;
    if (x > r || y < l) return INF;
    if (x <= l && r <= y) return cur->val;
    int m = (l + r) / 2;
    return min(getMin(cur->lp, l, m, x, y), getMin(cur->rp, m + 1, r, x, y));
}
int solve() {
    int n, r;
    cin >> n >> r;
...
//        DBN(h, vh[h]);
        for (int v : vh[h]) {
            update(roots[h + 1], 1, n, tin[v], a[v]);
        }
    }
    int q;
    cin >> q;
    ...
    while (q--) {
        ...
        int x = ((p + last) % n);
        int k = ((q + last) % n) + 1;
//        DBN(x, k, h[x] + k, tin[x], tout[x]);
        last = getMin(roots[min(h[x] + k, n)], 1, n, tin[x], tout[x]);
        cout << last << '\n';
    }
    return 0;
}