...
//		for(auto &u:vec[i])
//		{
//			u=lower_bound(all(lsh),u)-lsh.begin();
//		}
//	}
//	int s=lsh.size();
//	memset(dp,-0x3f,sizeof(int)*(s+1));
//	for(int i=0;i<=s;i++) edges[i].clear();
//	for(int i=0;i<n;i++)
//	{
//		int pre=-1;
//		t.clear();
...
//		{
//			edges[t[j]-1].emplace_back(t.back(),t.size()-j);
//		}
//	}
//	dp[0]=0;
//	for(int i=0;i<s;i++)
//	{
//		for(auto &[j,w]:edges[i]) chmax(dp[j],dp[i]+w);
//		chmax(dp[i+1],dp[i]);
//	}
//	cout<<dp[s]<<endl;
//}
