...
struct FastIO {
	...
	template<isint(Int)> void read(Int &x) {
		x = 0;
		char c;
		bool neg = false;
		while (true) {
	...
	template<isfloat(Float)> void read(Float &n) {
		bool neg = false;
	...
	template<isint(Int)> void print(Int n) {
		pint(n);
	}
	
	template<isfloat(Float)> void print(Float n) {
		if (n < 0) {
			pcu('-');
			n = -n;
		}
