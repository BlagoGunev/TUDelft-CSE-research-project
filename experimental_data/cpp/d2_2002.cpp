...
void ahcorz(){
    ...
    vi pos(n);
    rep(n) pos[a[i]]=i;
    vc<multiset<int>> st(n);
    rep(i,1,n) st[tree.par[i]].insert(pos[i]);
    auto check=[&](int u){
        if(st[u].empty()) return true;
        return pos[u]<*st[u].begin();
    };
    ll tot=0;
    rep(n) tot+=tree.dis(a[i],a[(i+1)%n]);
    vi val(n);
    rep(n) val[i]=check(i);
    int tot2=0;
    rep(n) tot2+=val[i]==0;
    while(q--){
        int x,y; cin >> x >> y; x--,y--;
        set<int> st1={(x+n-1)%n,x,(y+n-1)%n,y};
        set<int> st2={tree.par[a[x]],a[x],tree.par[a[y]],a[y]};
        for(int i: st1){
            tot-=tree.dis(a[i],a[(i+1)%n]);
        }
        for(int i: st2){
            if(i>=0) tot2-=val[i]==0;
        }
        for(int i: {a[x],a[y]}){
            if(tree.par[i]>=0) st[tree.par[i]].erase(st[tree.par[i]].find(pos[i]));
        }
        swap(a[x],a[y]);
        swap(pos[a[x]],pos[a[y]]);
        for(int i: {a[x],a[y]}){
            if(tree.par[i]>=0) st[tree.par[i]].insert(pos[i]);
        }
        for(int i: st2){
            if(i>=0){
                val[i]=check(i);
        ...
        print(a[0]==0&&tot==(n-1)*2&&tot2==0?"YES":"NO");
        //bug(tot2);
    }
}
