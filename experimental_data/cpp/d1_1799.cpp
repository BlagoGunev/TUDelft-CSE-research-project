...
template <class T, class T2> inline int chkmin(T &a, T2 b) {
  return a > b ? a = b, 1 : 0;
}
template <class T, class T2> inline int chkmax(T &a, T2 b) {
  return a < b ? a = b, 1 : 0;
}
template <class T, class T2> inline T mmin(T a, T2 b) { return a < b ? a : b; }
template <class T, class T2> inline T mmax(T a, T2 b) { return a > b ? a : b; }
template <class T, class T2> inline void srt(T &a, T2 &b) {
  if (a > b)
    swap(a, b);
}
template <class T> inline T aabs(T a) { return a < 0 ? -a : a; }
template <class T, class... T2> inline T mmin(T a, T2... b) {
  return mmin(a, mmin(b...));
}
template <class T, class... T2> inline T mmax(T a, T2... b) {
...
public:
  mint() { _val = 0; }
  mint(ll value) { _val = value % Mod; }
  mint(ll value, const bool &&ok) { _val = value; }
  ...
  explicit operator ll() const { return _val; }
...
inline mint inv(const mint &a) { return pw(a, Mod - 2); }
...
template <class num> int RMQ<num>::lg[MAX];

template <class num> struct FEN {
  int n;
  num *bit;

  inline int mnbit(const int &x) { return x & -x; }
  FEN(const int &N) {
    n = N;
    bit = new int[n + 5];
  }
...
template <class Node> struct SEG {
  ...
  template <class Ty> SEG(const int &N, const Ty *A) {
    n = N + 1;
    node = new Node[n << 1];
  ...
  template <class Ty> void upd(int i, const Ty &x) {
    i += n;
    node[i].upd(x);
  ...
  Node get(int l, int r) {
    l += n, r += n;
    Node lv, lvt, rv, rvt;
    Node *lres = &lv, *lres_t = &lvt;
    Node *rres = &rv, *rres_t = &rvt;
    for (; l < r; l >>= 1, r >>= 1) {
      if (l & 1)
...
template <class T> struct AdditionNode {
  T x = 0;
  ...
  template <class Ty> void upd(const Ty &u) { x += u; }
  static void merge(const AdditionNode<T> &a, const AdditionNode<T> &b,
                    AdditionNode<T> &res) {
    res.x = a.x + b.x;
  }
};
...
void solve() {
  ...
  // dp[i][j] => dp[mx + 1][j] , dp[i][mx + 1]

  for (int i = 0; i < n; i++)
    for (int j = 0; j < n; j++) {
      int mx = max(i, j);
      dp[mx + 1][j] =
          min(dp[mx + 1][j], dp[i][j] + (A[mx + 1] == A[i] ? hot[A[mx + 1]]
                                                           : cold[A[mx + 1]]));
      dp[i][mx + 1] =
          min(dp[i][mx + 1], dp[i][j] + (A[mx + 1] == A[j] ? hot[A[mx + 1]]
                                                           : cold[A[mx + 1]]));
    }
  ll ans = INF;
  for (int i = 0; i < n; i++)
