...
int t,n,m,a[200005],b[200005],d[200005];
map<int,int>mp1,mp2;
signed main(){
	...
	while(t--){
		...
		cin>>m;
		for(int i=1;i<=m;i++)cin>>d[i],mp2[d[i]]++;
		for(int i=1;i<=n;i++)if(a[i]!=b[i])mp1[b[i]]++;
		int flag=1,ff=0;
		for(auto v:mp1){
			if(v.second>mp2[v.first])flag=0;
		}
		if(!flag){
			cout<<"NO\n";continue;
		}
		for(int i=1;i<=n;i++)if(b[i]==d[m])ff=1;
		cout<<((ff)?"YES\n":"NO\n");
	}
}