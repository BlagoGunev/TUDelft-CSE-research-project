...
stack<int> up, down, upos, dpos;
int cnt[MX], used[MX], ara[MX], ans[MX];
...
//    FAST_READ
    ...
    for1(i,n) {
        if(!up.size()) {
            ...
            if(ara[i]>up.top()) {
                ...
                while(up.size() && used[upos.top()]>1 && ara[i]<= up.top()) {
                    up.pop();
                    used[upos.top()]--;
                    upos.pop();
                }
                if(!up.size() || ara[i]>up.top()) {
                    up.push(ara[i]);
                    upos.push(i);
                    used[i]++;
                }
        ...
        if(!down.size())  {
            ...
            if(ara[i]<down.top()) {
                ...
                while(down.size() && used[dpos.top()]>1 && ara[i]>= down.top()) {
                    down.pop();
                    used[dpos.top()]--;
                    dpos.pop();
                }
                if(!down.size() || ara[i]<down.top()) {
                    down.push(ara[i]);
                    dpos.push(i);
                    used[i]++;
                }
        ...
        // cout<<"i "<<i<<" uptop "<<up.top()<<" downtop "<<down.top()<<endl;
        if(!used[i]) {
            cout<<"NO"<<endl;
            return 0;
        }
    ...
    // cout<<"up_size "<<upos.size()<<" down_size "<<dpos.size()<<endl;

    while(dpos.size()) {
        // cout<<"dpos_top "<<dpos.top()<<endl;
        ans[dpos.top()]=1;
        dpos.pop();
    }
