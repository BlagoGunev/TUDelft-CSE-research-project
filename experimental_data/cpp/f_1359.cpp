        ...
        for(int j=i+1;j<n;j++)
        {
            if(dx[i]*dy[j]==dx[j]*dy[i])
            {
                if(dx[i]*(y[j]-y[i])==dy[i]*(x[j]-x[i]))
                {
                    ...
                    if(eff_s!=0)
                    {
                        s1=sqrt((y[j]-y[i])*(y[j]-y[i])+(x[j]-x[i])*(x[j]-x[i]))/eff_s;
                        if(ans==-1||ans>s1)
                        {
                            ans=s1;
                        }
            ...
            {
                s1=(sqrt(dx[i]*dx[i]+dy[i]*dy[i])*(dy[j]*(x[j]-x[i])-dx[j]*(y[j]-y[i])))/(s[i]*(dx[i]*dy[j]-dx[j]*dy[i]));
                t2=(sqrt(dx[j]*dx[j]+dy[j]*dy[j])*(dy[i]*(x[i]-x[j])-dx[i]*(y[i]-y[j])))/(s[j]*(dx[j]*dy[i]-dx[i]*dy[j]));
                if(s1>=0&&t2>=0&&(ans==-1||ans>max(s1,t2)))
                {
                    ans=max(s1,t2);
                }
            }
