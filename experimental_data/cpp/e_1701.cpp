...
int main() {
  ...
  for (int testcases = $, testcase = 0; testcase++ < testcases; ) {
    ...
    auto v = match(s, t), u = match({s.rbegin(), s.rend()}, {t.rbegin(), t.rend()});
    ...
    for (int offset = 0; offset <= s.size() - t.size(); ++offset)

    for (int i = 0; i < t.size() && i < ans - 2 * offset; ++i) if (int j = i + offset; t[i] == s[j] && v[i] <= j) {

      int d = 1;

      while (d < t.size() - i && t[i + d] == s[j + d]) {

        ++d;

      }

      if (u[t.size() - i - d] <= s.size() - j - d) {

        mini(ans, offset + !!j + s.size() - d);
