/** Micro Mezzo Macro Flation -- Overheated Economy ., Last Update: Feb. 11th 2013 **/ //{
...
int main() {
    ...
    RD(n);
    REP(i, n) A.PB(MP(RD(), i));
    REP(i, n) A.PB(MP(RD(), i)); RD(MOD);

    Fac[0] = 1, Two[0] = 0;
    REP_1(i, 2*n){
        int t = i; Two[i] = Two[i-1];
    ...
    REP(i, SZ(A)){
        int j = i+1;
        while (j<2*n && A[j].fi == A[i].fi)++j;

        //cout << i << " " << j << endl;
        //cout << j-i << endl;
        MUL(res1, Fac[j-i]); res2 += Two[j-i]; i = j - 1;
    }
    //cout << res2 << endl;

    res2 -= (2*n - SZ(UNQ(A)));
    OT(pdt(res1, pow(2, res2)));
}