...
minmaxS minmaxop(minmaxS l, minmaxS r) { return {min(l.first, r.first), max(l.second, r.second)}; }
minmaxS minmaxe() { return {inf, -inf}; }
...
void solve() {
    ...
    cin >> N >> Q >> S;
    dbg(S);
    vector<minmaxS> diffs(N - 1);
    REP(i, N - 1) {
        int v = abs(S.at(i + 1) - S.at(i));
        diffs.at(i) = {v, v};
    }
    atcoder::segtree<minmaxS, minmaxop, minmaxe> segtree(diffs);

    constexpr int D = 26;
    vector cs(D, vector<int>(N + 1));
    REP(i, N) {
        REP(d, D) {
            cs.at(d).at(i + 1) = cs.at(d).at(i) + (S.at(i) == 'a' + d);
        }
    }

    auto pars = enumerate_palindromes(S);

    vector<int> c2len(N);
    vector<int> c2lenodd(N);
    for (auto [l, r] : pars) {
        const int len = r - l;
        ...
        if (len % 2 == 0) {
            const int c = (l + r) / 2;
            chmax(c2len.at(c), len);
        } else {
            const int c = (l + r) / 2;
            chmax(c2lenodd.at(c), len);
        }
    }
    dbg(c2len);
    ...
    REP(q, Q) {
        ...
        const auto p = segtree.prod(l, r - 1);

        lint ans = 0;

        int types = 0;
        REP(d, D) {
            if (cs.at(d).at(r) - cs.at(d).at(l) > 0) {
                ++types;
            }
        }
        ...
        if (len == 1) {
            ...
            // aaa...aaa
            ans = 0;
        } else if (types == 2 and p.first == p.second) {
            // ababa...
            // 2 + 4 + ... + last
            lint init = 2, last = len / 2 * 2;
            lint num = (last - init) / 2 + 1;
            ans = num * (init + last) / 2;

            if (len % 2 == 0) may_red = true;
        } else {
            // assert(false);
            // 2 + ... + len
            ans = (lint)len * (len + 1) / 2 - 1;
        ...
        if (may_red) {
            const int c = (l + r) / 2;
            if (len % 2 == 0) {
                if (c2len.at(c) >= len) ans -= len;
            } else {
                if (c2lenodd.at(c) >= len) ans -= len;
            }
        }
        cout << ans << '\n';
    }
