...
void ahcorz(){
    ...
    int k=-1;
    vc<pii> res;
    auto right_rotate=[&](int i){
        res.pb({i+1,i+2});
        rotate(a.begin()+i,a.begin()+i+(k-1),a.begin()+i+k);
    };
    auto left_rotate=[&](int i){
        res.pb({i+2,i+1});
        rotate(a.begin()+i,a.begin()+i+1,a.begin()+i+k);
    };
    {
        int cnt=0;
        rep(n-1) if(a[i+1]!=a[i]+1) cnt++;
        if(cnt==0){
            print(n);
            print(0);
            return;
    ...
    int cnt=0;
    rep(n) rep(j,i+1,n) if(a[j]<a[i]) cnt^=1;
    if(cnt&&(n%2==0)){
        k=n-2;
        if(a[0]==n-1) right_rotate(0);
        if(a[1]==n-1) right_rotate(1);
        while(a[n-1]!=n-1) right_rotate(2);
    }
    else k=n-1;
    cnt=0;
    rep(n) rep(j,i+1,n) if(a[j]<a[i]) cnt^=1;
    ...
    auto double_right_rotate=[&](int t){ // t 1 2 ... t-1 n t+1 t+2 ... n-1
        assert(t>=1&&t<=k);
        rep(k-(t-1)) right_rotate(0);
        rep(t) right_rotate(1);
    };
    auto double_left_rotate=[&](int t){
        assert(t>=1&&t<=k);
        rep(t) left_rotate(1);
        rep(k-(t-1)) left_rotate(0);
    };
    auto left_shift=[&](int t){ // ... t-1 t ... n -> ... t n ... t-1
        double_right_rotate(t);
        double_left_rotate(t-1);
    };
    auto right_shift=[&](int t){
        bug(t);
        double_right_rotate(t-1);
        double_left_rotate(t);
    };
    cnt=0;
    rep(n) rep(j,i+1,n) if(a[j]<a[i]) cnt^=1;
    assert(cnt==0);
    rep(k-1) if(a[i]!=i){
        int pos=-1;
        rep(j,i,k+1) if(a[j]==i){
            pos=j;
            break;
        }
        assert(pos>i);
        if(pos<k) right_shift(pos+1);
        right_shift(i+2);
        assert(a[i]==i);
    }
    rep(n) assert(a[i]==i);
