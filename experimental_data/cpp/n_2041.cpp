...
int solve(int ban){
	...
	repp(i,st,n){
		...
		//if(v[i].size()==0&&i!=st)pa[i] = st,col[i] = col[st],rlt += a[i]+a[st];
		if(!pa[i]){
			++p;
			if(p==ban)++p;
			if(p<i){
...
/*
g++ N.cpp -o N -g -std=c++14 -Wall -Wshadow -fsanitize=undefined,address && ./N < in.in
ulimit -s unlimited
*/