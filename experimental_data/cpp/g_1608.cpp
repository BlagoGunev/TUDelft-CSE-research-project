...
vector<size_t> solve_2d(const vector<tuple<int,int,int,int>> &queries, const vector<int> &values) {
	...
	for(size_t k=0; k<qn; ++k) {
		...
		q.emplace_back(l, -(k+1));
		q.emplace_back(r, +(k+1));
		//for(int i=sl; i<sr; ++i) if(l<=values[i] && values[i] < r) ++ans[k];
	}
...
void run_case(const size_t ____test) { // rr(____test)
	...
	hasher f(str_hld);
	hasher fr(rbegin(str_hld), rend(str_hld));
	...
	for(int i=0; i<qn; ++i) {
		...
		auto sr = partition_point(begin(sa)+sl, end(sa), [&](int i) {
			// return way is prefix of suf[i]
			return lcp(way, suf(i)) == way.length();
		}) - begin(sa);
	...
	for(int i=0; i<size(sa); ++i) pval[i] = id[sa[i]];
