...
int N, K, Q;
vector<int> A, B;
vector<int> single_count, double_count;
vector<vector<mod_int>> answers;

void solve(int k) {
    int position = lower_bound(A.begin(), A.end(), B[k]) - A.begin();
    int single_cnt = single_count[position];
    int double_cnt = double_count[position];
    answers[k] = NTT::mod_multiply(NTT::mod_power({1, 2}, single_cnt), NTT::mod_power({1, 2, 1}, double_cnt));
}
...
int main() {
    ...
    sort(A.begin(), A.end());
    sort(B.begin(), B.end());
    single_count.assign(N + 1, 0);
    double_count.assign(N + 1, 0);

    for (int i = 0, j = 0; i < N; i = j) {
        while (j < N && A[i] == A[j])
            j++;

        single_count[j] = single_count[i];
        double_count[j] = double_count[i];
    ...
    for (int q = 0; q < Q; q++) {
        ...
        for (int k = 0; k < K; k++) {
            int goal = (perimeter - 2 * B[k]) / 2 - 1;

            if (goal < 0 || goal >= (int) answers[k].size())
                continue;

            total += answers[k][goal];
        }
